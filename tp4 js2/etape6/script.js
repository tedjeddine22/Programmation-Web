...
function deleteRow() {
		...
		let lignes = table.rows.length - 1; 
		if (lignes === 0) {
				alert("Le tableau est vide !");
				return;
		}

		let checkboxes = document.querySelectorAll(".col_chkbox input[type='checkbox']");
		
		let auMoinsUne = false;
		checkboxes.forEach(c => { if (c.checked) auMoinsUne = true; });
		if (!auMoinsUne) {
		...
		let total_points = 0;
		let summary_total = document.getElementById("p3");

		for (let i = checkboxes.length - 1; i >= 0; i--) {
				if (checkboxes[i].checked) {
						let row = checkboxes[i].parentNode.parentNode;  // ligne du tableau

						let pts = parseInt(row.cells[3].innerText);
						total_points += pts;
		...
		alert("Les lignes sélectionnées ont été supprimées avec succès !");
		update_summary();
}

function update_summary() {
		let lignes = persons.length;
		document.getElementById("p1").innerText = lignes + " ligne(s)";

		let somme = 0;
		for(let p of persons){
				somme += parseInt(p.points);
		}
		document.getElementById("p3").innerText = "Totale point(s) = " + somme;
}